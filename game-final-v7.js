import { supabase, isRemoteEnabled } from "./supabase-local.js";
...
function showGame() {
  ...
  switchView("office");
  if (isRemoteEnabled && state?.userMode === "creator" && state?.studioId) {
    syncStudioToRemote({ id: state.studioId, name: state.studioName, type: state.studioType, checkInTime: state.checkInTime, officeLayout: state.officeLayout }).then(() => syncRemoteStudios()).catch(() => {});
  }
  if (!state.deskId) {
    setTimeout(() => {
...
document.getElementById("studio-search-button").addEventListener("click", async () => { await syncRemoteStudios(); renderStudioSearchResults(document.getElementById("studio-search-input").value); });
...
startButton.addEventListener("click", async () => {
  ...
  if (creator) {
    ...
    await registerStudio({ id: profile.studioId, name: profile.studioName, type: selectedProfession, owner: profile.nickname, checkInTime: "09:00" });
...
async function initRemoteAuth() {
  ...
  async function finishAuthenticated(session) {
    ...
    startScreen.hidden = true;
    gameScreen.hidden = true;
    try { await syncRemoteStudios(); } catch (error) { console.error(error); }
    showStartExperience();
  }

  authForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    if (!supabase) { authStatus.textContent = "Supabase 未初始化，无法登录。"; return; }
    authStatus.textContent = "正在登录...";
    try {
      const { data, error } = await supabase.auth.signInWithPassword({ email: authEmail.value, password: authPassword.value });
      if (error) { authStatus.textContent = "登录失败：" + error.message; return; }
      await finishAuthenticated(data.session);
    } catch (error) {
      authStatus.textContent = "登录异常：" + String(error.message || error);
    }
  });

  document.getElementById("auth-signup").addEventListener("click", async () => {
    if (!supabase) { authStatus.textContent = "Supabase 未初始化，无法注册。"; return; }
    ...
    try {
      ...
      if (error) { authStatus.textContent = "注册失败：" + error.message; return; }
      if (data.session) await finishAuthenticated(data.session);
      else authStatus.textContent = "注册成功，请检查邮箱完成验证。";
    } catch (error) {
      authStatus.textContent = "注册异常：" + String(error.message || error);
    }
  });
  ...
  if (!isRemoteEnabled) {
    ...
    startScreen.hidden = true;
    gameScreen.hidden = true;
    authStatus.textContent = globalThis.supabase
      ? "Supabase 配置无效：请检查 studio-config.js 中的 Publishable key。"
      : "Supabase 本地库未加载：请检查 supabase-umd.js 是否上传成功。";
    return;
  }
  ...
  startScreen.hidden = true;
  gameScreen.hidden = true;
  authStatus.textContent = "正在读取登录状态...";

  try {
    const sessionResult = await Promise.race([
      supabase.auth.getSession(),
      new Promise((_, reject) => setTimeout(() => reject(new Error("读取登录状态超时")), 8000))
    ]);
    if (sessionResult?.data?.session) await finishAuthenticated(sessionResult.data.session);
    else authStatus.textContent = "请输入邮箱和密码登录或注册。";
  } catch (error) {
    authStatus.textContent = "读取登录状态失败：" + String(error.message || error);
  }
}
...
function registerStudio(studio) {
  if (isRemoteEnabled) {
    ...
    return syncStudioToRemote(studio);
  }
...
async function syncStudioToRemote(studio) {
  ...
  if (!data?.user) return { ok: false, reason: "未登录" };
  ...
  if (error) {
    ...
    showToast("工作室同步失败：" + error.message);
...
async function syncRemoteStudios() {
  ...
  if (error) {
    console.error("Studio list sync failed", error);
    showToast("工作室列表加载失败：" + error.message);
    return;
  }
...
document.getElementById("goal-form").addEventListener("submit", (event) => {
  ...
  const dueTime = document.getElementById("goal-time").value || null;
  state.workGoals.push({ id: createId("goal"), title, location: document.getElementById("goal-location").value, dueTime, done: false, rewarded: false, reminded: false, nextReminderAt: dueTime ? null : Date.now() + (10 + Math.random() * 10) * 60000 });
  state.logs.push({ time: formatClock(), text: "新增工作目标：" + title + "。" });
  event.target.reset();
  saveState();
  renderAll();
});
document.getElementById("tasks-view-list").addEventListener("click", (event) => {
  const button = event.target.closest("[data-goal-id]");
  if (button) toggleWorkGoal(button.dataset.goalId);
});
document.getElementById("office-map").addEventListener("click", (event) => {
  const staffTarget = event.target.closest("[data-staff-id]");
  if (staffTarget && state) {
    event.stopPropagation();
    const staff = state.staff.find((item) => item.id === staffTarget.dataset.staffId);
    if (state.asleep && staff) wakeByReminder(staff, staff.name);
    else openSocialDialog(staffTarget.dataset.staffId);
    return;
  }
  const agentTarget = event.target.closest("[data-agent-id]");
  if (agentTarget && state) {
    event.stopPropagation();
    const agent = state.agents.find((item) => item.id === agentTarget.dataset.agentId);
    if (agent && state.asleep) wakeByReminder(agent, agent.name);
    else if (agent) showToast(`${agent.name} 正在${agent.location}自动工作，进度 ${Math.round(agent.progress || 0)}%`);
    return;
  }
  const room = event.target.closest("[data-location]");
  if (!room || !state) return;
  activateLocation(room.dataset.location);
});

actionList.addEventListener("click", (event) => {
  const button = event.target.closest("[data-action-task]");
  if (!button || button.disabled) return;
  completeTask(button.dataset.actionTask);
});

document.getElementById("close-action-drawer").addEventListener("click", () => {
  activeLocation = null;
  actionDrawer.hidden = true;
});

document.getElementById("dismiss-reminder").addEventListener("click", () => {
  reminderMutedUntil = Date.now() + 15 * 60 * 1000;
  renderReminder();
});

document.getElementById("next-day-button").addEventListener("click", finishToday);
document.getElementById("reset-button").addEventListener("click", resetGame);
document.getElementById("collection-button").addEventListener("click", () => {
  renderCollection();
  collectionDialog.showModal();
});
document.getElementById("close-collection").addEventListener("click", () => collectionDialog.close());
document.querySelectorAll("[data-view]").forEach((button) => {
  button.addEventListener("click", () => switchView(button.dataset.view));
});
document.getElementById("reward-store-list").addEventListener("click", (event) => {
  const button = event.target.closest("[data-redeem-id]");
  if (!button) return;
  redeemReward(button.dataset.redeemId);
});
document.getElementById("staff-list").addEventListener("click", (event) => {
  const button = event.target.closest("[data-staff-action]");
  if (!button) return;
  handleStaffAction(button.dataset.staffAction, button.dataset.staffId);
});
document.getElementById("task-form").addEventListener("submit", (event) => {
  event.preventDefault();
  if (state.userMode !== "creator") return;
  const assigneeId = document.getElementById("task-assignee").value;
  const assignee = state.staff.find((item) => item.id === assigneeId);
  state.publishedTasks.push({ id: createId("task"), title: document.getElementById("task-title").value.trim(), assignee: assignee?.name || assigneeId, points: Number(document.getElementById("task-points").value) || 10, type: document.getElementById("task-type").value });
  state.logs.push({ time: formatClock(), text: `发布新任务：${state.publishedTasks.at(-1).title}。` });
  event.target.reset();
  saveState();
  renderAll();
  showToast("新任务已发布");
...
document.getElementById("meeting-form").addEventListener("submit", (event) => {
  ...
  const check = canScheduleMeeting(meetingTime);
  if (!check.ok) {
    showToast("预约失败：" + check.reason);
    return;
  }
  state.meetings.push({ id: createId("meeting"), topic: document.getElementById("meeting-topic").value.trim(), time: meetingTime, dateKey: getTodayKey(), status: "scheduled" });
  state.logs.push({ time: formatClock(), text: `已安排会议：${state.meetings.at(-1).topic}。` });
  event.target.reset();
  saveState();
  renderAll();
  showToast("会议已安排");
});
document.getElementById("reward-form").addEventListener("submit", (event) => {
  event.preventDefault();
  if (state.userMode !== "creator") return;
  const type = document.getElementById("reward-type").value;
  const reward = { id: createId("reward"), name: document.getElementById("reward-name").value.trim(), cost: Number(document.getElementById("reward-cost").value) || 100, stock: Number(document.getElementById("reward-stock").value) || 1, type, description: "由创作工作室手动添加的兑换奖品。", skin: type === "skin" ? document.getElementById("reward-skin").value : null };
  state.rewards.push(reward);
  state.logs.push({ time: formatClock(), text: `新增兑换奖品：${reward.name}。` });
  event.target.reset();
  saveState();
  renderAll();
  showToast("新奖品已加入商店");
...
document.getElementById("agent-form").addEventListener("submit", (event) => {
  event.preventDefault();
  if (state.userMode !== "creator") return;
  state.agents.push({ id: createId("agent"), name: document.getElementById("agent-name").value.trim(), role: document.getElementById("agent-role").value.trim() || "通用 Agent", skill: document.getElementById("agent-skill").value.trim() || "待配置", location: "core", taskState: "working", status: "Working", progress: 0, mood: 80, body: 80, ability: 50, points: 0, isAgent: true });
  state.logs.push({ time: formatClock(), text: `新增虚拟员工：${state.agents.at(-1).name}。` });
  event.target.reset();
  saveState();
  renderAll();
  showToast("虚拟员工已增加");
...
document.querySelectorAll("[data-layout-mode]").forEach((button) => {
  button.addEventListener("click", () => setOfficeLayout(button.dataset.layoutMode));
});
document.getElementById("settings-reset").addEventListener("click", resetGame);
document.getElementById("reset-confirm-check").addEventListener("change", (event) => {
  document.getElementById("reset-confirm-button").disabled = !event.target.checked;
});
document.getElementById("reset-cancel").addEventListener("click", () => document.getElementById("reset-dialog").close());
document.getElementById("reset-confirm-button").addEventListener("click", performReset);
document.getElementById("attendance-form").addEventListener("submit", (event) => {
  event.preventDefault();
  if (state.userMode !== "creator") return;
  state.checkInTime = document.getElementById("check-in-time").value || "09:00";
  const directory = loadStudioDirectory().map((studio) => studio.id === state.studioId ? { ...studio, checkInTime: state.checkInTime } : studio);
  saveStudioDirectory(directory);
  state.logs.push({ time: formatClock(), text: "考勤签到时间设置为 " + state.checkInTime + "。" });
  saveState();
  renderAll();
  showToast("签到时间已保存：" + state.checkInTime);
});
document.getElementById("patrol-button").addEventListener("click", startPatrol);
document.getElementById("clockout-close").addEventListener("click", () => { document.getElementById("clockout-overlay").hidden = true; });

document.getElementById("agenda-go").addEventListener("click", goToAgendaLocation);
document.getElementById("agenda-later").addEventListener("click", () => { document.getElementById("agenda-dialog").close(); activeAgendaRule = null; });
document.getElementById("close-social-dialog").addEventListener("click", () => document.getElementById("social-dialog").close());
document.querySelector(".social-actions").addEventListener("click", (event) => {
  const button = event.target.closest("[data-social-action]");
  if (button) {
    if (button.dataset.socialAction === "meeting") {
      renderSocialOptions("meeting");
      scheduleSocialMeeting();
    } else {
      renderSocialOptions(button.dataset.socialAction);
    }
  }
});
document.getElementById("social-option-panel").addEventListener("click", (event) => {
  const gift = event.target.closest("[data-gift-id]");
  const chat = event.target.closest("[data-chat-id]");
  const emoji = event.target.closest("[data-emoji]");
  if (gift) sendGift(gift.dataset.giftId);
  else if (chat) sendChat(chat.dataset.chatId);
  else if (emoji) sendEmoji(emoji.dataset.emoji);
});
